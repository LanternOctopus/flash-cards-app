...
const WordSpan: React.FC<WordSpanProps> = ({
    ...
    const [wasClickedState, setWasClickedState] =
        useState<boolean>(false);

    const wasClicked = () => {
        setWasClickedState(true);
        if (isCorrect) {
            handleWordsSelected(word);
        } else {
            incrementWrongAnsCount();
        }
    };
    ...
    const borderColor = wasClickedState
        ? isCorrect
            ? "3px solid green"
            : "3px solid red"
        : "none";
    const icon = wasClickedState
        ? isCorrect
            ? "✅"
            : "❌"
        : "";
    word =
        wasClickedState && word.includes("'")
            ? " " + normalizeWord(word)
            : word;
...
const PartofSpeechView: React.FC<PartsOfSpeechProps> = ({
    data,
    updateSuccess,
}) => {
    ...
    useEffect(() => {
        if (
            data.answer.length === wordsSelected.length &&
            data.answer
                .slice()
                .sort()
                .every(
